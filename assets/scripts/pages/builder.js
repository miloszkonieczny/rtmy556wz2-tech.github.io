...
import {
  ...
  postMinimalWaitlistToFormspree,
} from "../services/formspree.js";
import {
  fillFormFromProfile,
  getFormProfile,
} from "../services/story-profile.js?v=20260724-story-fix";
import {
  ...
  try {
    ...
    selector.addEventListener("change", () => {
      const selectedProfile = profiles.find(
        (profile) => profile.id === selector.value,
      );
...
function clearStepErrors(step) {
  ...
  step
    .querySelectorAll(".field-error, .group-error")
    .forEach(clearElementMessage);
}
...
function validateStep(step) {
  ...
  radioNames.forEach((name) => {
    if (!step.querySelector(`input[name="${name}"]:checked`)) {
      setElementMessage(
        step.querySelector(`[data-error-for="${name}"]`),
        "form.error.choice",
      );
      isValid = false;
    }
  ...
  const missingCheckbox = Array.from(
    step.querySelectorAll("input[type='checkbox'][required]"),
  ...
  if (missingCheckbox) {
    ...
    setElementMessage(
      step.querySelector(
        `[data-error-for="${missingCheckbox.name}"]`,
      ) || step.querySelector('[data-error-for="consent"]'),
      "form.error.consent",
    );
    isValid = false;
  }
...
function initializeBuilderPage() {
  ...
  const generateButton =
    form.querySelector(".wizard-generate");
  const progressFill =
    document.querySelector("#progress-fill");
  const progressLabels = Array.from(
    document.querySelectorAll(".progress-labels span"),
  );
  const formStatus =
    document.querySelector("#form-status");
  const marketingConsent =
    form.elements.marketingConsent;
  const emailField = form.elements.email;
  let currentStep = 0;
  ...
  function syncMarketingEmailRequirement() {
    ...
    emailField
      .closest(".field")
      ?.classList.toggle("is-required", emailField.required);
  }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    generateButton.textContent =
      translate("builder.status.creating");
    ...
    if (marketingConsent?.checked && emailField?.value) {
      ...
      try {
        await postMinimalWaitlistToFormspree(
        ...
        console.warn(
          "MoonTale waitlist submission failed.",
          error,
        );
      }
    }
